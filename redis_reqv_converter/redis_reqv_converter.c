...
int process_get(char* key, char** pg_answer, int* size_pg_answer){
    ...
    //ereport(DEBUG1, errmsg("START GET"));
    if(res == NON){
        //ereport(DEBUG1, errmsg("IN process_get: GET non key: %s",key));
        *pg_answer = (char*)malloc( sizeof(char));
        if(*pg_answer == NULL){
            ereport(ERROR, errmsg("can't malloc process_get: pg_answer"));
            return -1;
        }
        (*pg_answer)[0] = 3;
        *size_pg_answer = 1;
        ...
        //ereport(DEBUG1, errmsg("IN process_get: GET  key: %s value: %s", key, value));
       ...
       // ereport(DEBUG1, errmsg("pg_answer: %s value: %s size_pg_answer: %d ", *pg_answer, value, *size_pg_answer));
    }
    //ereport(DEBUG1, errmsg("FINISH GET"));
    return 0;
}
...
int process_set(char* key, char* value, char** pg_answer, int* size_pg_answer){
    ...
    //ereport(DEBUG1, errmsg("START SET"));
    if(res == ERR_REQ){
        ...
        *size_pg_answer = 4;
        ...
        (*pg_answer)[2] = 'K';
        (*pg_answer)[3] = '\0';
        //ereport(DEBUG1, errmsg("IN process_set: SET key:%s value: %s", key, value));
    }
    return 0;
}
...
int process_del(int command_argc, char** command_argv, char** pg_answer, int* size_pg_answer) {
    ...
    //ereport (DEBUG1, errmsg("START DEL, %d arguments", command_argc));
    ...
    *pg_answer = (char*)malloc(  *size_pg_answer * sizeof(char));
    ...
    //ereport(DEBUG1, errmsg("FINISH DEL, answer: %s", *pg_answer));
    return 0;
}

int process_ping(char** pg_answer, int* size_pg_answer){
    //ereport(DEBUG1, errmsg("IN process_ping"));
    *size_pg_answer = 6;
    *pg_answer = (char*)malloc( *size_pg_answer  * sizeof(char));
    if(*pg_answer == NULL){
...
int process_command(int command_argc, char** command_argv) {
    //ereport(DEBUG1, errmsg("IN process_command"));
    return 0;// plug
...
int process_redis_to_postgres(int command_argc, char** command_argv, char** pg_answer, int* size_pg_answer) {
    //ereport(DEBUG1, errmsg("PROCESSING STARTED %d", command_argc));
    if (command_argc == 0) {
    ...
    if (!strcmp(command_argv[0], "GET")) {
        ...
        //ereport(DEBUG1, errmsg("GET_PROCESSING: %s", command_argv[0]));
        ...
        //ereport(DEBUG1, errmsg("SET_PROCESSING: %s", command_argv[0]));
        ...
        //ereport(DEBUG1, errmsg("COMMAND_PROCESSING: %s", command_argv[0]));
        return 0;
